...
4. **pip 설치는 허용 — 단 오프라인 불변식을 지키는 조건부**(2026-07-04 사용자 승인):
   - **개발/집/리뷰 PC에서는 필요한 라이브러리를 `pip install`로 자유롭게 설치**해
     SKIP 대신 실제 경로를 검증하라(예: `pip install openpyxl` → xlsx 실파싱 분기 실행).
   - 추가한 패키지는 **반드시 `release/dependencies.json`에 등록**(status +
     `pip download … -d release/vendor/…` prefetch + offline_install). 회사 PC는
     **오프라인**이라 런타임 pip이 없다 — deps는 P17-02/03 번들의 prefetch된 wheel로만
     반입된다(`pip install --no-index --find-links …`). "dev에서 pip 됐다"를 "회사에서도
     된다"로 착각 금지.
   - **코어(`agent_ops/` 비어댑터부)는 여전히 stdlib-only**: python-embed + wheel 0개로
     구동돼야 한다(dependencies.json runtime_note). 외부 패키지는 어댑터/ingest 확장에만.
   - 어댑터/ingest는 **optional-import + 부재 시 정직한 안내**(graceful degradation)를
     유지하라 — 회사·타 PC에 아직 그 wheel이 없을 수 있다. 라이브러리 유무 양쪽 분기를
     테스트가 커버하면 이상적(bench의 openpyxl 유/무 분기가 예시).
