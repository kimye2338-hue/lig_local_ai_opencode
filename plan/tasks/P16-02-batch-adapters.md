...
2. **(r2→r3) exit-53 fake의 POSIX 바이트 emit**: 리눅스 `/bin/sh`(dash)는 `printf '\xNN'`을
   해석 안 함 → 리터럴 출력 → log_tail 깨져 `"ErrorStatus=53" in log_tail` 실패로 파일
   POSIX RED. 수정: unix_body를 **octal `\NNN`**로
   (`\377\376E\000r\000r\000o\000r\000S\000t\000a\000t\000u\000s\000=\0005\0003\000`,
   Python 소스에선 `\\377…`), 또는 log_tail 내용 assertion만 `os.name=="nt"` 가드하고
   classification(ok False·`exit 53`·returncode 53)은 크로스플랫폼 유지.
   (reviews/P16-02-r2.md "되는 방법" — 둘 다 리뷰어 검증됨.)
...
>   분류해 안내, ④ `/p <회사프로파일> /product ACADM`(사용자 실행 방식)은 acad.exe용 —
