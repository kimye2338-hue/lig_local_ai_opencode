...
- 2026-07-05 **후속 5건 (Fable 직접, 같은 브랜치)**: ① 브라우저 신뢰성 — 활성 탭 고정/SPA 렌더 대기/script 소스 누출 차단/spa_map 루트 복귀/`fill` 액션, 헤드리스 Chromium 라이브 E2E 13종 실측 후 test_browser_adapter 라이브 섹션으로 회귀화(26 checks). ② **LLM Wiki**(Karpathy 패턴) — 주제 페이지 복리 통합(`wiki_manager.py`)+[[링크]]+index/log/schema+lint+선택 LLM 큐레이션+지식책 위키 챕터, recall 에 페이지 발췌 주입(test_wiki_manager 27). ③ **모드 3개 정리** — build/plan(내장)+`agent`(supervisor/autopilot/plan 통합), 설치·패치가 구모드 파일 삭제, CI 필수파일 검사 갱신. ④ **회상 보장** — pinned_recall(사용자 규칙+최근 실수는 무조건 주입)+루프 실패 자동 기억(test_recall_guarantee 7). ⑤ 펫 오버레이 이미지화(사용자 스티커 6종)+에이전트 루프 라이브 상태 발행, IME 밀림 완화(런처 wt 우선, 실측 대기). 설치 E2E(리눅스 --home) 재실측: ocd/ai/agent.md/구모드정리 전부 정상. 전 스위트 green(Windows 전용 3 제외).

- 2026-07-05 **FABLE-OCD-WORKSPACE-PROFILES 구현 완료 + FABLE-PRODUCT-DIRECTION 1~6 착지 (Fable 직접)**: ① `ocd` 폴더-로컬 프로필(`.opencodelig` 시드, 편집본 절대 미덮어쓰기) + 전역 기억 공유 — `project_profile.py`(단일 소스)/`ocd.py`/`bin\ocd.bat·ai.bat`(PATH). ② `run_agent_loop` 컨텍스트 주입: 전역 recall → 폴더 기억/페르소나/규칙 + 충돌 규칙. ③ LLM tool `project_info`/`remember` 노출(구현≠노출 교훈, 스키마 예산 7.8KB 갱신). ④ doctor `project_profile` 섹션. ⑤ `run_cmd` UTF-8→CP949 폴백(mojibake 수정, RUNTIME_LESSONS §4). ⑥ PR #10 잔여 결함 수정: test_browser_adapter 가 구 5-액션 고정이라 RED → SPA 세트 subset 검증으로 갱신. ⑦ **패치파일 체계 신설**: `release/build_patch.py` → `OpenCodeLIG_PATCH_*.zip`(프로그램만 교체, 변경분 백업, USERDATA 불가침) — 설치파일(build_bundle)과 병행 빌드. 회귀: 신규 test_ocd_profiles 25 + test_patch_build 21, 전 스위트 green(리눅스, py 셔임 포함 26/29 실행 — RED 3은 Windows 전용). 남은 것: 회사 PC에서 `ocd` 실주행.
