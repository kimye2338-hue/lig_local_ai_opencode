...
WS-7 정책 엔진(`auto_policy.py`) + 사용자 선택 최소화. cmd_auto가 실행 전 execute/plan_only/ask_user/blocked를 결정. **safety 우회 불가, 모델 기본값 변경 없음.** 첫 파일: `workspace/agent_ops/auto_policy.py`, `workspace/agent_ops/agentops.py`, `workspace/tests/test_auto_policy.py`.
