...
WS-5(구 WS-INT)는 TUI/Python/ocd가 같은 경로·환경을 보게 통합한다(AGENTOPS_HOME 기준). **한 커밋으로 묶고, 모델 기본값·opencode.json env 보간 2단계는 사내망 검증 필요**. 첫 파일: `workspace/RUN_OPENCODE_LIG.bat`, `workspace/.opencode/commands/*.md`, `workspace/tests/test_launch_bats.py`.
