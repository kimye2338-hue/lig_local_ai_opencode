...
## WS-8 완료 (2026-07-08, 커밋 afe7f59)

- 새로 연결된 지능: cmd_auto 실행 후 score_run이 6개 축으로 채점 → diagnostics/evaluations.jsonl append(성장 근거). route_preferences/growth_report로 관측. maintenance:evaluation_loop 등록(166).
- 보호한 안전: append-only + USERDATA 미접촉, min_samples=3 과잉승격 금지, auto_policy/memory_manager 미변경, cmd_auto 기존코드 무수정.
- 검증: test_evaluation_loop(29), test_auto_command(27), test_auto_policy(23), test_intelligence_map(166), work_command(4) PASS.

## WS-9 완료 (2026-07-09, 커밋 ebfd08c)

- 새로 연결된 지능: `memory_quality`가 등급 분리 + promote/decay/dedupe(비파괴)로 recall 품질을 정제. auto_maintain 스로틀에 배선. recall이 user_rule/preference/project_fact 우선, 비user activity ×0.5 감쇠. memory:memory_quality 등록(167).
- 보호한 안전: apply_quality는 status/priority/tags만, 행 삭제 없음(ids 불변 가드), protected 미접촉, memory.jsonl append 무변경.
- 검증: test_memory_quality(35), test_recall_guarantee(7), test_adapter_tools_maintain(24), test_intelligence_map(167) PASS.

---

# WS-10 전체 지능망 최종 리뷰 (2026-07-09)

## 전체 테스트 결과 (전부 PASS)
intelligence_map 167 · auto_command 27 · routing_alignment 9 · auto_policy 23 · evaluation_loop 29 · auto_learning_hooks 18 · memory_quality 35 · opencode_config 27 · think_strip 10 · tool_dispatch 28 · knowledge_routing 42 · wiki_manager 41 · memory_activity 7 · recall_guarantee 7 · recall_stemming 9 · adapter_tools_maintain 24 · memory_inject_plugin 23 · opencode_command_coverage 25 · launch_bats 101 · capability_bench 222 · pytest work_command 4 · doctor exit 0. (release/ 요구·실LLM/Office 스모크는 배포패키지에 소스 없어 스킵/실패가 정상.)

## auto route trace 8종 실증 (dry-run, USERDATA·게이트웨이 미접촉, 전부 exit 0 / safety_bypass=false / eval 6축 기록)

| 시나리오 | selected_path | command | policy | 비고 |
| --- | --- | --- | --- | --- |
| 문서 작성 | artifact | work | execute | document_generation+weekly_report |
| 데이터/HTML 리포트 | artifact | work | execute | spreadsheet_generation |
| 일정/비서 | tool_agent | agent | ask_user | ▲튜닝: 일정등록은 되돌릴 수 있어 execute가 이상적(과잉질문) |
| Obsidian/위키/회상 | memory_wiki | recall | execute | 별칭·manual recall 연결 |
| 앱/파일 조작 | tool_agent | agent | execute | file_ops 우선 |
| 공학/전공 KB | plan_only | plan | plan_only | ▲튜닝: "계산" 요청은 tool_agent(KB 주입 후 답)가 더 나음 |
| 반복 루틴 | command_native | routine_list | execute | command-native 위임 |
| 위험/애매(삭제) | tool_agent | agent | ask_user | 확인필요(requires_confirmation) — 의도된 안전 정지 |

각 trace에 model_provider(unchanged), capability_ids, policy(mode/requires_confirmation/reason/fallback), effective_mode(실행 시), context_sources, verification, memory_hooks, evaluation(6축), safety(approval_bypass=false)가 기록된다. → **요청→라우팅→정책→(실행)→완료후크→평가**가 오프라인 닫힌 루프로 작동.

## 최종 판정
- **미연결 지능 0**: test_intelligence_map 167 — 모든 command/tool/adapter/capability/context/memory/maintenance 항목이 auto/advanced/pending/deprecated 중 하나로 분류·고정. 고아 없음.
- **안전 우회 0**: 8종 trace 전부 approval_bypass=false. auto_policy는 safety를 `max(base,floor)` 단방향으로만 강등(승격 불가). command_guard/승인/deny 무변경.
- **USERDATA 위험 변경 0**: 신규 적재/평가/품질은 전부 append-only 또는 status/priority/tags 갱신. memory_quality는 행 개수 불변 가드 + protected(user_rule/preference/manual) 미접촉. memory.jsonl append 최적화는 계측 우선으로 미룸.
- **GitHub push 0**: 로컬 커밋만(이 세션 17커밋). origin push 없음.
- **모델 기본값 임의 변경 0**: opencode.json 기본값 Qwen vibe_think_off 유지, tool-confirmed 모델만 additive 노출. 기본값 변경은 사용자 A/B 후.

## 사내망 필요 체크리스트 (오프라인 검증 불가 — 사용자/다음 작업자)
1. **WS-5 실행/설정 통합**: bash 셸(cmd/POSIX) 확인 → 48개 커맨드 `.md` 경로 정규화 + ocd cwd 복원 + compaction-handoff 경로 + bat env 따옴표(한 커밋). env 보간 2단계·모델 기본값 단일화. (절차: AUTO_ORCHESTRATION_PLAN 9절 WS-5.)
2. **모델 A/B**: TUI에서 `lig-exaone-chat`(EXAONE 대화용) 우선 비교 → 최선을 기본값 고정.
3. **TUI 플러그인 실동작**: memory-inject(세션시작 recall 주입)·compaction-handoff·log-activity 실제 훅 발화.
4. **라우팅 튜닝(위 ▲)**: 일정등록 과잉질문·공학KB plan_only — 실사용 관측 후 `_auto_command_hint`/`auto_policy` 신호 조정.

## 상태: WS-0 ~ WS-10 구현·테스트·문서·로컬커밋 완료. 오프라인 검증 가능 범위 전부 green. 사내망 실기기 항목은 위 체크리스트로 분리됨.
