...
WS-6 자동 유지보수/효율화(오프라인 가능). consolidate/book/lint 계측 + 반복 실패 error_pattern 승격(opt-in) + remember 재생성 중복 감소. 첫 파일: `workspace/agent_ops/auto_maintain.py`, `workspace/agent_ops/activity_timeline.py`, `workspace/tests/test_adapter_tools_maintain.py`.
