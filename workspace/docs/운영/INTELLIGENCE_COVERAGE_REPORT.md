...
WS-4는 사용자가 Obsidian에 수동으로 남긴 노트(`wiki/manual/*.md`)도 자동 recall 대상에 포함하되, 원장으로 역주입하지 않고 원본을 보존한다. 첫 파일: `workspace/agent_ops/wiki_manager.py`, `workspace/tests/test_wiki_manager.py`.
