...
### Obsidian으로 보기 (LLM 위키)

**LLM 위키란?** 비서가 대화 중 배운 것(사용자 규칙·업무 교훈·정리한 지식)을 `remember`/`book`/`wiki`로
저장하면, 그 기억이 **자동으로 Markdown 노트가 되어** `%USERPROFILE%\OpenCodeLIG_USERDATA\memory\wiki`에
쌓입니다. Obsidian은 이 폴더를 그래프·백링크로 보기 좋게 보여주는 **뷰어**일 뿐 — 없어도 위키(.md)는
정상 동작하고, 있으면 연결 관계를 시각적으로 볼 수 있습니다.

**설치·연결 (한 번만):**

1. **Obsidian 준비** — 둘 중 하나. 설치 파일(약 295MB)은 이 패키지에 없으니 사내 반입 절차로 가져오세요.
   - *일반 설치*: 받은 `Obsidian-x.x.x.exe`를 그냥 실행해 설치(오프라인 가능). → 자동 인식됨.
   - *포터블*: `Obsidian.exe`를 아래 위치에 두기(설치 불필요).
     ```text
     %USERPROFILE%\OpenCodeLIG\tools\Obsidian\Obsidian.exe
     ```
2. **위키 열기** — 아래 런처를 더블클릭하면 **끝**입니다.
   ```text
   %USERPROFILE%\OpenCodeLIG\workspace\launch\wiki.bat
   ```
   이 런처가 자동으로: (a) vault 폴더를 만들고 최소 설정을 시드(`.obsidian`이 없을 때만),
   (b) 포터블 → 설치본 순으로 Obsidian을 찾아 **위키 폴더를 열어줍니다**. Obsidian이 없으면
   탐색기로 위키 폴더만 엽니다. 사용자가 바꾼 Obsidian 설정은 덮어쓰지 않습니다.

즉 **설정은 자동**입니다 — Obsidian만 준비해서 `wiki.bat`을 누르면 됩니다. 상세: `docs\기능\OBSIDIAN_WIKI.md`.
