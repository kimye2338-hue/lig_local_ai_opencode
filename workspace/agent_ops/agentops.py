...
def cmd_schedule(args):
    ...
    if action == "add":
        text = " ".join(args.text).strip()
        due_text = str(getattr(args, "due", "") or "").strip()
        parsed = schedule_store.parse_due(due_text or text)
        if not parsed.get("ok"):
            print(SCHEDULE_DUE_HINT, file=sys.stderr)
            return 2
        title = text if due_text else _schedule_clean_title(text)
        result = schedule_store.add(title, due_text or text)
...
def main(argv=None):
    ...
    sp = sched.add_parser("add"); sp.add_argument("text", nargs="+"); sp.add_argument("--due", default=""); sp.set_defaults(func=cmd_schedule)
