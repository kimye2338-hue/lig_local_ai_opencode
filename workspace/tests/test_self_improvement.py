...
def test_failure_then_success_creates_actionable_lesson(monkeypatch, tmp_path: Path) -> None:
    ...
    lessons = _read_jsonl(tmp_path / "lessons.jsonl")
    assert [e["kind"] for e in events] == ["self_error", "self_fix"]
    assert len(lessons) == 1
    assert lessons[0]["kind"] == "self_lesson"
    assert "다음에는" in lessons[0]["next_action"]
    assert "subagent" in lessons[0]["failure"]
    assert lessons_for_injection(limit=3)[0]["id"] == lessons[0]["id"]


def test_self_improvement_injection_is_limited_and_formatted(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("LIG_SELF_IMPROVEMENT_DIR", str(tmp_path))
    from agent_ops.self_improvement import format_injection_block, record_fix
