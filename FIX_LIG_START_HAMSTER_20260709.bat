...
rem 관리자 권한 필요(hosts 수정). 아니면 자동 승격.
net session >nul 2>&1
if errorlevel 1 (
  echo 관리자 권한으로 다시 실행합니다... (UAC 창에서 예)
...
rem PURE 재활성(플러그인 꺼짐) 대신, 외부 조회 호스트를 hosts로 즉시 실패시켜 지연만 제거.
echo.
echo [1/3] 외부 모델조회 호스트 차단(hosts) - 백업 후 추가
...
rem ===== 문제2b: 런처 grace 300->60 (문제1 해결로 긴 대기 불필요, 종료후 뮤텍스 빨리 해제) =====
echo.
echo [3/3] 런처 시작유예 300초 -> 60초: %LAUNCHER%
