...
from .core import ROOT, AGENT_OPS, REPORTS, RESULTS, MEMORY, run_cmd, atomic_write_text, atomic_write_json, read_text, read_json
...
def verify() -> dict:
    heartbeat("verify")
    try:
        from .memory_manager import ensure_memory
        ensure_memory()
    except Exception:
        pass
    ...
    required = [
        ...
        ROOT / ".opencode" / "agents" / "agent.md",
        ROOT / ".opencode" / "commands" / "continue.md",
        MEMORY / "memory.jsonl",
    ]
