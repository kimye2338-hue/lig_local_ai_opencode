...
def plan_execution(artifact_kinds, files, input_paths=(), probe_only=False):
    ...
    for kind in artifact_kinds or []:
        ...
        elif kind == "ansys_script":
            add(kind, "ansys", _first_with_suffix(files, ".py"), False,
                "app validation pending: ANSYS Mechanical/SpaceClaim GUI 스크립팅 콘솔에서 수동 실행 검증 필요")
        else:
            # slide_outline / browser_script / mail_report 등:
