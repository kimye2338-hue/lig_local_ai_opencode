...
def _document_type(path: Path) -> int:
    return SW_DOC_TYPES.get(path.suffix.lower(), 0)
...
def run_macro(doc_path: str, bas_path: str, options: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    try:
        errors = 0
        warnings = 0
        doc_type = _document_type(copy_doc)
        if doc_type == 0:
            return {"ok": False, "copy_path": str(copy_doc), "error": "지원하지 않는 SolidWorks 문서 확장자"}
        model = app.OpenDoc6(str(copy_doc), doc_type, 1, "", errors, warnings)
        if model is None:
            return {"ok": False, "copy_path": str(copy_doc), "error": "SolidWorks 문서 열기 실패"}
        module = str(opts.get("module") or opts.get("module_name") or "")
        procedure = str(opts.get("procedure") or opts.get("procedure_name") or "main")
        ok = bool(app.RunMacro2(str(macro), module, procedure, 0, 0))
